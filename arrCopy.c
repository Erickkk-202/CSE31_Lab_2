#include<stdio.h>
#include<string.h>
#include<stdlib.h>

void printArr(int *a, int size, char *name){
	//Your code here
    for(int i = 0; i< sizeof(name); i++){
        printf("%c", *(name+i));// prints out each letter of the string
    }
    printf(" array’s contents: ");
    
    
    for(int i = 0; i < size; i++){
        printf(" %d ", *(a+i)); //prints out the array
    }
    printf("\n");
}

int* arrCopy(int *a, int size){
	//Your code here
    int *copy;
    copy = (int *) malloc( sizeof(int) * size); //dynamic allocates memory for copy

   for (int i =0; i< size;i++){
       *(copy+i) = (*(a+i)); // copies array 
         
   }
   return copy;// returns copy
}

int main(){
    int n;
    int *arr;
    int *arr_copy;
    int i;
    printf("Enter the size of array you wish to create: ");
    scanf("%d", &n);

    //Dynamically create an int array of n items
    //Your code here
   
    arr = (int *) malloc( sizeof(int) * n);//dynamic allocates memory for arr


    //Ask user to input content of array
	//Your code here

    for(i =0; i< n;i++){
         printf("Enter array element  #%d: ", (i+1));
        scanf("%d", arr + i); // inputs numbers.
        
        
    }
    
   
    
	
/*************** YOU MUST NOT MAKE CHANGES BEYOND THIS LINE! ***********/
	
	//Print original array
    printArr(arr, n, "Original");

	//Copy array
    arr_copy = arrCopy(arr, n);

	//Print new array
    printArr(arr_copy, n, "Copied");

    printf("\n");

    return 0;
}